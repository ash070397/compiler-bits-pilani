...
ASTNode *postorder(tree_node *pTree, ASTNode *inh){
   ...
   if(pTree->is_terminal==0 && pTree->enum_value==NT_program){
        ...
        strcpy(ast_PROGRAM->data.str, "driver");
        ...
        ast_PROGRAM->data.start_line = moduleDef_syn->data.start_line;
        ast_PROGRAM->data.end_line = moduleDef_syn->data.end_line;
        ...
        //
        ast_MODULE->data.start_line = moduleDef_syn->data.start_line;
        ast_MODULE->data.end_line = moduleDef_syn->data.end_line;
        ...
        //returned in moduleDef as a stmts node..
        tree_node *start_stmts = pTree->child;
        tree_node *end_stmts = pt_statements->next;
        ast_statements->data.end_line = end_stmts->line_number;
        ast_statements->data.start_line = start_stmts->line_number;
   ...
   //ASHISH PART-1 ENDS
    ...
    //6. N7 logicalOp AnyTerm N7 | EPS ? (changed)
    if(pTree->is_terminal == 0 && pTree->enum_value == NT_N7){
        if(pTree->branch_no == 0){
            ...
            ASTNode *Anyterm_syn = postorder(PT_anyterm, NULL);
            //ASTNode *N7_2_syn = postorder(PT_n7_2, Anyterm_syn);
            ASTNode *N7_2_syn = postorder(PT_n7_2, Logical_op_syn);
            ...
            //
            Logical_op_syn->right_child = Anyterm_syn;
            //

            //ASTNode *N7_1_syn = Logical_op_syn;
            ASTNode *N7_1_syn= N7_2_syn;
            return N7_1_syn;
        }
        else if(pTree->branch_no == 1){
            //N7.syn = N7.inh
    ...
    //8. N8 relationalOp arithmeticExpr | EPS ? (no change)
    ...
    if(pTree->is_terminal == 0 && pTree->enum_value == NT_N4){
        if(pTree->branch_no == 0){
            ...
            ASTNode *Term_syn = postorder(PT_term, NULL);
            ASTNode *N4_2_syn = postorder(PT_n4_2, Op1_syn);
            ...
            //
            Op1_syn->right_child = Term_syn;
            //
            
            //ASTNode *N4_1_syn = Op1_syn;
            ASTNode *N4_1_syn = N4_2_syn;
            return N4_1_syn;
        }
        else if(pTree->branch_no == 1){
            //N4.syn = N4.inh
    ...
    //13. N5 op2 factor N5 | EPS ? (changed)
    if(pTree->is_terminal == 0 && pTree->enum_value == NT_N5){
        if(pTree->branch_no == 0){
            ...
            ASTNode *Factor_syn = postorder(PT_factor, NULL);
            ASTNode *N5_2_syn = postorder(PT_N5_2, Op2_syn);
            ...
            //
            Op2_syn->right_child = Factor_syn;
            //

            //ASTNode *N5_1_syn = N5_2_syn;
    ...
    if(pTree->is_terminal == 0 && pTree->enum_value == NT_conditionalStmt && pTree->branch_no == 0)
    {
        ...
        ASTNode *default_syn = postorder(PT_default,AST_default); 
        ...
        tree_node *start_switch = PT_ID->next->next;
        tree_node *end_switch = PT_default->next;
        AST_SWITCH->data.start_line = start_switch->line_number;
        AST_SWITCH->data.end_line = end_switch->line_number;
    ...
    if(pTree->is_terminal == 0 && pTree->enum_value == NT_default && pTree->branch_no == 0)
    {
        ...
        inh->data.line_number = pTree->child->line_number;
    ...
    if(pTree->is_terminal == 0 && pTree->enum_value == NT_iterativeStmt && pTree->branch_no == 0)
    {
        ...
        tree_node *start_for = PT_range->next->next;
        tree_node *end_for = PT_statements->next;
        AST_FOR->data.start_line = start_for->line_number;
        AST_FOR->data.end_line = end_for->line_number;
    ...
    if(pTree->is_terminal == 0 && pTree->enum_value == NT_iterativeStmt && pTree->branch_no == 1)
    {
        ...
        tree_node *start_while = PT_AOB->next->next;
        tree_node *end_while = PT_statements->next;
        AST_WHILE->data.start_line = start_while->line_number;
        AST_WHILE->data.end_line = end_while->line_number;
...
void printTree(ASTNode *current){
    ...
    if(current->right_child == NULL){
        if(current->data.is_terminal == 1){
            printf("Terminal: %s, ",terminal_map[current->data.enum_value]);
            if(current->data.enum_value == T_ID){
                printf("Variable name: %s, ", current->data.str);
            }
